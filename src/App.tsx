...
export default function App() {
  ...
  const page = route === 'author' ? <AuthorPage sessionId={sessionId} /> : route === 'host' ? <HostPage sessionId={sessionId} /> : <StudentPage sessionId={sessionId} />
  return <><header className="site-header"><a className="brand" href={href('student')}><span className="brand-mark">Q</span><span>Quantum Circuit Classroom</span></a><nav><a className={route === 'student' ? 'active' : ''} href={href('student')}>Student</a><a className={route === 'author' ? 'active' : ''} href={href('author')}>Author</a><a className={route === 'host' ? 'active' : ''} href={href('host')}>Host</a></nav></header>{route === 'student' ? page : <TeacherAuthGate>{page}</TeacherAuthGate>}<footer>Quantum Circuit Classroom · Session {sessionId}</footer></>
}
